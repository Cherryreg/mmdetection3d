...
class FCAF3DHead(BaseModule):
    ...
    def _forward_single(self, x, scale):
        ...
        for permutation in x.decomposition_permutations:
            ...
            center_preds.append(center_pred.index_select(0, permutation))
            # bbox_preds.append(bbox_pred[permutation])
            bbox_preds.append(bbox_pred.index_select(0, permutation))
            # cls_preds.append(cls_pred[permutation])
            cls_preds.append(cls_pred.index_select(0, permutation))
    ...
    def _loss_single(self, center_preds, bbox_preds, cls_preds, points,
        ...
        pos_center_preds = center_preds.index_select(0, pos_inds)
        # pos_bbox_preds = bbox_preds[pos_inds]
        pos_bbox_preds = bbox_preds.index_select(0, pos_inds)
